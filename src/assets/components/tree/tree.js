...
const Tree = function(container) {
  ...
  const openItem = this.container.find('.jui-tree__item--open');
  if (openItem[0]) {
...
Tree.prototype.getNextItem = function() {
  ...
  if (current.hasClass('jui-tree__item')) {
    ...
    if (!next[0]) {
      const parentTreeItem = current.parents('[role=treeitem]');
...
Tree.prototype.getPreviousItem = function() {
  ...
  if (current.hasClass('jui-tree__item')) {
    // Previous folder
    const previousFolder = current.prev('[role=treeitem]');
    if (previousFolder) {
