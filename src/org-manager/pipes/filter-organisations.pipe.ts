...
export class FilterOrganisationsPipe implements PipeTransform {
  ...
  public transform(orgs: OrganisationVM[], searchFilter: string): OrganisationVM[] {
    ...
    return orgs.filter((org: OrganisationVM) => {
      return org && (org.name && org.name.toLowerCase().includes(searchFilter)) ||
      ...
      (org.dxNumber && org.dxNumber.length >= 1 && org.dxNumber[0] && org.dxNumber[0].dxNumber && org.dxNumber[0].dxNumber.toLowerCase().includes(searchFilter) ) ||
      (org.dxNumber && org.dxNumber.length >= 1 && org.dxNumber[0] && org.dxNumber[0].dxExchange && org.dxNumber[0].dxExchange.toLowerCase().includes(searchFilter));
    });
  }
}
