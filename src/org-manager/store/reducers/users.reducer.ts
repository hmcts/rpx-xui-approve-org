	...
	switch (action.type) {
		...
		case fromActions.SUBMIT_REINVITE_USER_ERROR: {
			const errorMessages = {
				serverResponse: {
					messages: [
						action.payload.error.apiStatusCode === 409 && AppConstants.ERROR_MESSAGE_MAPPINGS
							? AppConstants.ERROR_MESSAGE_MAPPINGS[1]
		...
		case fromActions.SUBMIT_REINVITE_USER_ERROR_CODE_429: {
			const errorMessages = {
				...
				serverResponse2: {
					messages: [
						'The recipient will receive an email from HM Courts and Tribunals Registrations so they can finish setting up their account',
					],
				},
			};
			return {
