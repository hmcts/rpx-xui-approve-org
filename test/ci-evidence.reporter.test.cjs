...
test('archives canonical evidence before HTML publication in verified hooks', () => {
  const archive = "steps.archiveArtifacts allowEmptyArchive: true, artifacts: 'functional-output/tests/playwright-*/odhin-report/xui-ci-evidence.json'";
  const cnp = fs.readFileSync(path.resolve(__dirname, '..', 'Jenkinsfile_CNP'), 'utf8');
  const nightly = fs.readFileSync(path.resolve(__dirname, '..', 'Jenkinsfile_nightly'), 'utf8');
  const hooks = [
    [
      cnp,
  ...
  const reportDir = cnp.match(/String playwrightReportDir = '([^']+)'/)?.[1];
  const archiveGlob = archive.match(/artifacts: '([^']+)'/)?.[1];
  assert.ok(reportDir, 'missing playwrightReportDir');
  assert.ok(archiveGlob, 'missing archive glob');
  const actualJsonPath = `${reportDir}/xui-ci-evidence.json`;
  const [globPrefix, globSuffix] = archiveGlob.split('*');
  assert.ok(actualJsonPath.startsWith(globPrefix) && actualJsonPath.endsWith(globSuffix), 'archive glob must match playwrightReportDir');

  for (const [source, startMarker, endMarker, mustPrecedeHtml] of hooks) {
    const start = source.indexOf(startMarker);
    const end = source.indexOf(endMarker, start);
    const hook = source.slice(start, end < 0 ? source.length : end);
    assert.ok(start >= 0, `missing hook: ${startMarker}`);
    assert.ok(hook.indexOf(archive) >= 0, `missing archive: ${startMarker}`);
    if (mustPrecedeHtml) {
      assert.ok(hook.indexOf(archive) < hook.indexOf('publishHTML'), `archive must precede HTML publication: ${startMarker}`);
    } else {
      assert.ok(hook.slice(hook.indexOf('{') + 1).trim().startsWith(archive), `archive must be first in hook: ${startMarker}`);
    }
  }
});
