...
const buildSmokePlaywrightArgs = (env = process.env, extraArgs = process.argv.slice(2)) => {
  const args = ['test', 'playwright_tests/e2e/login.test.ts', '-c', 'playwright.config.ts', ...extraArgs.filter((arg) => arg !== '--')];
  const smokeGloballyExcluded =
    !resolveBooleanFlag(env.PLAYWRIGHT_IGNORE_GLOBAL_EXCLUDES) &&
    splitTagInput(env.PLAYWRIGHT_GLOBAL_EXCLUDED_TAGS).includes('@smoke');
