...
test.describe(
  'Playwright API negative: all user list without roles',
  { tag: ['@all-user-list-without-roles', '@negative'] },
  () => {
    test('GET /api/allUserListWithoutRoles without auth is denied', async ({ apiAnonymousRequest }) => {
      ...
      expect([302, 401, 403], `Expected denied status to be one of 302/401/403. Received status=${httpStatus}`).toContain(
        httpStatus
      );
    ...
    test('GET /api/allUserListWithoutRoles with invalid usersOrgId returns error or empty list', async ({ apiRequest }) => {
      ...
      if (httpStatus === 200) {
        ...
        expect(httpStatus, `Expected error status for invalid org ID. Received status=${httpStatus}`).toBeGreaterThanOrEqual(400);
      }
    });
  }
);
