...
test.describe('Playwright API positive: organisations', { tag: ['@organisations', '@positive'] }, () => {
  for (const status of statuses) {
    test(`GET /api/organisations?status=${status} returns a paginated JSON list`, async ({ apiRequest }) => {
      const response = await apiRequest.get('/api/organisations', {
        params: {
          status,
          page: 1,
          size: ORGANISATION_SEARCH_PAGE_SIZE
        },
      ...
      expect(
        httpStatus,
        `Expected 200 from paginated GET /api/organisations?status=${status}. Received status=${httpStatus}`
      ).toBe(200);

      const contentType = resolveHeader(response.headers(), 'content-type');
      expect(
        contentType,
        `Expected JSON content-type for paginated GET /api/organisations?status=${status}. Received content-type=${contentType}`
      ...
      expect(payload.length).toBeLessThanOrEqual(ORGANISATION_SEARCH_PAGE_SIZE);
    });
  }

  test('GET /api/organisations?status= returns a paginated JSON list', async ({ apiRequest }) => {
    const response = await apiRequest.get('/api/organisations', {
      params: {
        status: '',
        page: 1,
    ...
    expect(
      httpStatus,
      `Expected 200 from paginated GET /api/organisations?status=&page=1&size=${ORGANISATION_SEARCH_PAGE_SIZE}. Received status=${httpStatus}`
    ).toBe(200);

    const contentType = resolveHeader(response.headers(), 'content-type');
    expect(
      contentType,
      `Expected JSON content-type for paginated GET /api/organisations?status=. Received content-type=${contentType}`
    ).toContain('application/json');
