import { test, expect, waitForOrganisationStatus } from './helpers/fixtures';
import { signIn } from './helpers/login';
import { navigateToOrganisationDetails, waitForOrganisationDetailsAction } from './helpers/navigate';
import { getTableDataByXpath } from './helpers/tables';

test('i can approve a pending org', async ({ page, organisationId, userName }) => {
  await signIn(page);
  await expect(page.getByRole('heading', { name: 'Organisation approvals' })).toBeVisible();
  await page.locator('#search').fill(`${userName}-company`);
  await page.getByRole('button', { name: 'Search' }).click();

  const detailsLink = page.locator(`a[href*="/organisation-details/${organisationId}"]`).first();
  await expect(detailsLink).toBeVisible({ timeout: 15_000 });
  await expect(page.getByText(`${userName}-company`).first()).toBeVisible();
...
test('i can reject a pending org', async ({ page, organisationId }) => {
  await signIn(page);
...
test('i can place registration under review for a pending org', async ({ page, organisationId }) => {
  await signIn(page);
  await navigateToOrganisationDetails(page, organisationId);
  await expect(page.getByRole('heading', { name: 'Approve organisation' })).toBeVisible();
  await expect(page.locator('app-org-details-info')).toBeVisible();
...
test('i can delete an active org', async ({ page, organisationId }) => {
  await signIn(page);
  await navigateToOrganisationDetails(page, organisationId);
  await expect(page.getByRole('heading', { name: 'Approve organisation' })).toBeVisible();
  const orgName = (await getTableDataByXpath(page, '//app-org-details-info/form/div/dl/div[1]/dd[1]')).trim();
  await expect(page.locator('app-org-details-info')).toBeVisible();
  await page.getByLabel('Approve it').check();
  await page.getByRole('button', { name: 'Submit' }).click();
  await expect(page.getByRole('heading', { name: 'Confirm your decision' })).toBeVisible();
  ...
  const activeOrganisationId = await waitForOrganisationStatus(page, orgName, 'ACTIVE', 180_000);
  await waitForOrganisationDetailsAction(page, activeOrganisationId, 'Delete organisation', 120_000);
