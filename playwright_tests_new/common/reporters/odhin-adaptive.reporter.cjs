...
class OdhinAdaptiveReporter {
  ...
  async onTestEnd(test, result) {
    ...
    const { nextResult, trimmedCounts } = trimResult(result, {
...
const trimResult = (result, options) => {
  ...
  const shouldTrimHeavyArtifacts = options.lightweight && passedOrSkipped;
  const shouldDropTestOutput =
    options.testOutputMode === false || (options.testOutputMode === 'only-on-failure' && passedOrSkipped);
...
OdhinAdaptiveReporter.__test__ = {
  normalizeTestOutputMode,
